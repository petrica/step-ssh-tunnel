...
KEEPALIVE="10"
if [ "$WERCKER_SSH_TUNNEL_KEEPALIVE" ]; then
  KEEPALIVE="$WERCKER_SSH_TUNNEL_KEEPALIVE"
fi
...
if  ! ssh -f -o ExitOnForwardFailure=yes -L "$SSH_TUNNEL" "$SSH_CONNECTION" "$SSH_PORT" sleep "\$KEEPALIVE"; then
